  ...
  <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-4">
        ...
        <select {...register('competidor', { required: true })} className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          ...
          {competidoresData?.map((competidor) => (
            <option key={competidor.id} value={competidor.id}>
              {`${competidor.Competidor_nombre} ${competidor.Competidor_apellido} - ${competidor.Competidor_dojo_nombre} - ${competidor.Categoria_nombre} - ${new Date(competidor.Campeonato_fecha).toLocaleDateString('es-ES', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric'
              })} ${new Date(competidor.Campeonato_fecha).toLocaleTimeString('es-ES', {
                hour: '2-digit',
                minute: '2-digit'
              })} `}
            </option>
        ...
        {errors.competidor && <span className="text-red-500 text-xs mt-1">Este campo es requerido</span>}
      </div>
      <div className="mb-4">
        ...
        {errors.motivo && <span className="text-red-500 text-xs mt-1">Este campo es requerido</span>}
      </div>
